/*
  BetaflightArduinoMavlink.cpp - Library for using Arduino to recieve Pixhawk's sensor data as well as some other usefull data which you might need.
      ...
      if(mavlink_parse_char(MAVLINK_COMM_0, ch, &msg, &status1)){
        ...
        switch(msg.msgid){
          ...
          case MAVLINK_MSG_ID_GPS_RAW_INT:
            {
              Serial.println("Recived GPS Data");
              break;
            }
          case MAVLINK_MSG_ID_ATTITUDE:
            {
              //Serial.println("Recived Att");
              mavlink_attitude_t data;
              mavlink_msg_attitude_decode(&msg, &data);
              //Serial.println(data.time_boot_ms);
              Serial.println(data.roll*57.295779513);
              //Serial.println(data.pitch*57.295779513);
              //Serial.println(data.yaw*57.295779513);
              break;
            }
